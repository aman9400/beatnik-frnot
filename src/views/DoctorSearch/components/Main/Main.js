...
//         className={styles.doctor_details_popup}
//       >
...
const Main = props => {
  ...
  const loadDoctorDetails = () => {
    ...
    fetch(
      'https://oaarogyabetaportal.mirakidigital.in/api/patient/home/index',
      requestOptions,
    )
      .then(response => response.json())
      .then(result => setDoctorDetails(result.doctors))
...
// Get address
  ...
  const getSlotByClinic = async(walkClinicId,doctorID) =>
  {
      ...
      var myHeaders = new Headers();
        ...
        var datas = await fetch(
          ...
          setGetSlotByClinics(datas.slots);
  ...
  const onDateCLickGetSchedule = async(slotDate,doctorID) =>
  {
    //setMyValue(slotDate)
  ...
  var myHeaders = new Headers();
    ...
    var datas = await fetch(
      'https://oaarogyabetaportal.mirakidigital.in/api/patient/appointments/calendarAppointments/2022-02-19',
      requestOptions,
    )
      .then(response => response.json())
      .catch(error => console.log('error', error));

      setFutureSLot(datas.slots)
  }
                          ...
                          <div className={styles.button_holder}>
                            <Button variant="contained">
                              ...
                              {service.appointment_type_id == '3' ? (
                               ...
                               {service.appointment_type_id == '4' ? (
                                <img onClick={e => getAddressOnHomeButton(recordData.id)} src={WalkIN} alt="#" />
                              ) : (
                                ''
                              )}
                            </Button>
                        ...
                        <>
                          <MuiPickersUtilsProvider utils={DateFnsUtils}>
                            <Grid
                              ...
                              justifyContent="space-around"
                            ></Grid>
                          </MuiPickersUtilsProvider> {myValue}
                          <FormGroup style={{ marginBottom: '20px' }}>
                            <TextField
                              mt={2}
                              id="date"
                              ...
                              name="myValue"
                              disablePast
                              onChange={(e) => onDateCLickGetSchedule(e.target.value,recordData.id,getAppointmentId)}
                              value={
                                myValue
                                  ? myValue
                                  : getslotdata.next_available_date
                      ...
                      <div className={styles.wrapper}>
                        <Grid container spacing={2}>
                          ...
                          >
                            ...
                            {/* {futureSlot ? futureSlot && futureSlot.map((showslot)=><p>{showslot.start_time}</p>) : 'sssssssssss'} */}
                            ...
                            { getslot ? (
                              ...
                              <div style={{ marginLeft: '330px' }}>
                               ...
                               {! getAddressOnHomeButton ? <b>No Slot Availble</b> 
                                 : getClinicAddres && getClinicAddres.map((data)=><button onClick={e => getSlotByClinic(data.id,recordData.id)}>
                                   
                                    <div style={{padding:"10px",border:'1px solid black'}}>{data.name}</div></button>
                               )}   
                              </div>
                            )}
